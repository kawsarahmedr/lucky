<?php
/**
 * The main core functional comments template for lucky theme.
 * 
 * @Package lucky
 * @since version 1.0.0
...
<div id="comments" class="comments-area">
	<?php if( have_comments() ): ?>
		<h2 class="comment-title">
			<?php
				printf(
					esc_html( _nx( 'One comment on &ldquo;%2$s&rdquo;', '%1$s comments on &ldquo;%2$s&rdquo;', get_comments_number(), 'comments title', 'lucky' ) ),
					number_format_i18n( get_comments_number() ),
					'<span>' . get_the_title() . '</span>'
				);		
			?>
		</h2>
		<?php lucky_post_comments_navigation(); ?>
	...
	<?php 
		...
		$args = array(
			'class_submit' => 'lucky-submit',
			'label_submit' => __( 'Submit Comment' ),
			'comment_field' =>
				'<div class="form-group"><label for="comment">' . _x( 'Comment', 'noun' ) . '</label> <span class="required">*</span><textarea id="comment" class="form-control" name="comment" rows="6" required="required"></textarea></div>',
